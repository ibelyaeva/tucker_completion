...
class TuckerTensorCompletionRunner(object):
    ...
    def init_variables(self):
        ...
        self.ground_truth_tf = tfe.Variable(tl.tensor(self.ground_truth))
        self.sparsity_mask_tf = tfe.Variable(tl.tensor(self.mask_indices))
        self.sparse_observation_tf = tfe.Variable(tl.tensor(self.sparse_observation))
        self.ten_ones_tf = tfe.Variable(tl.tensor(self.ten_ones))
    ...
    def init_algorithm(self):
        self.X = tfe.Variable(tl.tensor(self.x_init))
        self.X_old = tfe.Variable(tl.tensor(self.x_init))
        
        ranks = tu.get_tensor_shape_as_list(self.X)
        
        self.tensor_factors = []
        for i in range(len(self.X.get_shape()._dims)):
            item = tfe.Variable(self.create_factor((self.X.shape[i] , ranks[i])))
            self.logger.info("Factor Shape" + str(item.shape))
            self.tensor_factors.append(item)
            
        self.core  = tfe.Variable(tl.tensor(self.x_init))
